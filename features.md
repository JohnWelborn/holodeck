...
- **Closing instruction** (`closingInstruction`) — the final line of the user turn telling the LLM what to write. Use `{name}` for the character's name. Defaults to "Write {name}'s next response. Narrative prose — action and dialogue. If an action was proposed and agreed upon in the preceding exchange, perform it. Stop when their contribution is complete."
